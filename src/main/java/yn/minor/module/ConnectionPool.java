...
public class ConnectionPool {
	...
	Stack<Connection> standByPool = new Stack<Connection>();			// Standby state database connection pool
	Set<Connection> activePool = new HashSet<Connection>();			// Active state database connection pool
	...
	public void closeConnection() throws SQLException {
		for (int i=0; i<standByPool.size(); i++)
			standByPool.get(i).close();

		Iterator<Connection> iterator = activePool.iterator();
